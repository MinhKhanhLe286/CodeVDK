...
// Gas Sensor
const int GAS_SENSOR_PIN = A0; // Pin cảm biến gas
...
<body>
  ...
  <h1>Gas Sensor</h1>
  <p>Gas Sensor Value: <span id="gas_value"></span></p>
  <h1>Step Motor</h1>
  <form action="">
  ...
  server.on("/gas_value", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    request->send(200, "text/plain", String(gas_sensor_value));
  });
...
void loop() {
  // Xuất giá trị cảm biến gas ra Serial Monitor
  gas_sensor_value = analogRead(GAS_SENSOR_PIN);
  Serial.print("Gas Sensor Value (Loop): ");
  Serial.println(gas_sensor_value);
