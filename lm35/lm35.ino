...
const int TEMP_PIN = A0; // Chân tín hiệu của cảm biến nhiệt độ LM35
...
<body>
  ...
  server.on("/temperature", HTTP_GET, [](AsyncWebServerRequest *request) {
    // Đọc giá trị từ cảm biến nhiệt độ LM35
    int analogValue = analogRead(TEMP_PIN);
    float voltage = analogValue * 3.3 / 1024.0;
    float temperature = voltage * 100.0; // LM35 trả về 10mV/°C

    Serial.print("Temperature: ");
    Serial.print(temperature);
    Serial.println(" °C");

    request->send(200, "text/plain", String(temperature, 2));
  });
...
void loop() {
  // Đọc giá trị từ cảm biến nhiệt độ LM35 và xuất ra Serial Monitor
  int analogValue = analogRead(TEMP_PIN);
  float voltage = analogValue * 3.3 / 1024.0;
  float temperature = voltage * 100.0; // LM35 trả về 10mV/°C

  Serial.print("Temperature (Loop): ");
  Serial.print(temperature);
  Serial.println(" °C");
